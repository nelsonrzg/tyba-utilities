...
export class ExcelPdfService {
  private readonly logger = new Logger(ExcelPdfService.name);

  async generatePdfFromExcel(file: Express.Multer.File): Promise<{ outputPaths: string[] }> {
    if (!file) {
      throw new Error('Se requiere archivo Excel');
    }

    const workbook = new ExcelJS.Workbook();
    const inputBuffer = Buffer.isBuffer(file.buffer) ? file.buffer : Buffer.from(file.buffer as any);
    ...
    const formatNit = (value: any): string => {
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      // Extraer el NIT y obtener los primeros 4 dígitos para la contraseña
      ...
      selectedColumns.forEach((col) => {
        ...
        // Agregar espacio adicional a la izquierda para campos específicos
        ...
        // Aplicar negrilla a columnas específicas
      ...
      // Dibujar la imagen derecha a la altura de la tabla
      if (fs.existsSync(rightMarginImagePath)) {
        doc.image(rightMarginImagePath, 545, tableY + 292, { width: 20, height: 75 });
      }
      // Posicionar el texto debajo de la tabla en la misma página
      doc.y = tableY + 10 ;
